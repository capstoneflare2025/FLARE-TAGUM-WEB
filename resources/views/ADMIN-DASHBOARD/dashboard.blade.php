...
<div id="fireFighterChatSection" class="bg-white p-6 shadow rounded-lg mt-6 hidden">
  ...
  <div class="max-h-96 overflow-y-auto">
    <table class="min-w-full table-auto">
      ...
      <tbody id="fireFighterChatBody"></tbody>
    </table>
  </div>
...
(() => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try{
      ...
      await ref.set({ sender:'admin', text, timestamp: Date.now() });
...
function groupMessages(raw) {
  ...
  for (const m of raw) {
    ...
    const part =
      m.audioBase64 ? { ts, kind: 'audio', audioBase64: m.audioBase64 } :
      m.imageBase64 ? { ts, kind: 'image', imageBase64: m.imageBase64 } :
      m.text        ? { ts, kind: 'text',  text: m.text } :
                      { ts, kind: 'empty' };

    if (!bucket) {
      bucket = { type: m.type, timestamp: ts, parts: [part] };
      continue;
    }
  ...
  // sort each bucket’s parts chronologically (just in case)
  for (const b of out) b.parts.sort((a,b) => a.ts - b.ts);
...
function renderBubble(msg) {
  ...
  const canAppend = __lastBubble.el && __lastBubble.type === msg.type && (nowTs - __lastBubble.ts) <= __GROUP_WINDOW_MS;
  ...
  const parts = Array.isArray(msg.parts)
    ...
    : [
        ...
        msg.audioBase64 ? { ts: nowTs, kind:'audio', audioBase64: msg.audioBase64 } :
        msg.imageBase64 ? { ts: nowTs, kind:'image', imageBase64: msg.imageBase64 } :
        msg.text        ? { ts: nowTs, kind:'text',  text: msg.text } :
                          null
      ].filter(Boolean);
